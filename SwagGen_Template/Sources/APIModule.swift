...
extension {{ options.name }}{% call pathAsType currentPath %} {
    ...
    {% set type %}{% call pathTypeName path %}{% endset %}
    /// {{ path }}
    {% if name|contains:"{" %}
    public func callAsFunction(_ path: String) -> {{ type }} { {{ type }}(client: client(path)) }
    {% else %}
    public var {{ name|swiftIdentifier:"pretty"|lowerFirstWord|escapeReservedKeywords }}: {{ type }} { {{ type }}(client: client("{{name}}")) }
    {% endif %}
    public struct {{ type }} { public var client: APIClient }
}
