...
extension {{ options.name }}.API {
    ...
    public struct Server: Hashable {
        ...
        public static var `default` = {{ options.name }}.API.Server.{{ servers[0].name }}
        {% endif %}
        ...
        {% endif %}
        {% if server.variables %}
        public static func {{ server.name }}({% for variable in server.variables %}{{ variable.name|replace:'-','_' }}: String = "{{ variable.defaultValue }}"{% ifnot forloop.last %}, {% endif %}{% endfor %}) -> {{ options.name }}.API.Server {
            var urlString = "{{ server.url }}"
            {% for variable in server.variables %}
            urlString = urlString.replacingOccurrences(of: {{'"{'}}{{variable.name}}{{'}"'}}, with: {{variable.name|replace:'-','_'}})
            {% endfor %}
            return {{ options.name }}.API.Server(URL(string: urlString)!)
        }
        {% else %}
        public static let {{ server.name }} = {{ options.name }}.API.Server(URL(string: "{{ server.url }}")!)
        {% endif %}
        {% endfor %}
    }
}
...
extension APIClient.Configs {
    ...
    /// {{ options.name }} server
    public var {{ options.name|lowerFirstWord }}Server: {{ options.name }}.API.Server{% if servers[0].variables %}?{% endif %} {
...
extension {{ options.name }}.API{% call pathAsType currentPath %} {
