...
public extension AppleMusic.API {
	...
	func myAlbums(
		...
		let input = AppleMusic.API.MySongsInput(include: include, limit: limit ?? 100, offset: offset)
		return pages(limit: input.limit) { client in
			try await client.path("v1", "me", "library", "albums").query(input).get()
		}
	}
	...
	func myArtists(
		...
		let input = AppleMusic.API.MySongsInput(include: include, limit: limit ?? 100, offset: offset)
		return pages(limit: input.limit) { client in
			try await client.path("v1", "me", "library", "artists").query(input).get()
		}
	}
	...
	struct MySongsInput: Encodable, Sendable {
